...
import comfy.ops
import comfy.sd
from llada_image_t8.runtime import config as llada_config
from llada_image_t8.runtime.detection import detect_config
from llada_image_t8.runtime.loader import load_checkpoint
from types import SimpleNamespace
import comfy.utils
import nodes
from llada_image_t8.runtime.model import LLaDAImage
from llada_image_t8.runtime.text_encoder import LLaDAImageTEModel


def model_config_from_unet(state_dict, key_prefix, metadata=None):
    return llada_config.LLaDAImage(detect_config(state_dict, key_prefix, metadata))
...
def test_aio_file_loads_model_clip_and_vae_through_checkpoint_path(
    ...
    model, clip, vae = load_checkpoint(str(checkpoint), [])
...
def test_complete_tiny_aio_loads_and_runs_native_generation_and_editing(
    ...
    factory, factory_args, index = model.cached_patcher_init
    reloaded_model = factory(*factory_args, disable_dynamic=True)[index]
    assert not reloaded_model.is_dynamic()
    assert torch.equal(
        reloaded_model.model.diffusion_model.state_dict()[next(iter(expected_model))],
        expected_model[next(iter(expected_model))],
    )
    clip_factory, clip_args = clip.patcher.cached_patcher_init
    reloaded_clip = clip_factory(*clip_args, disable_dynamic=True)
    assert not reloaded_clip.is_dynamic()
    ...
    from llada_image_t8.nodes import (
